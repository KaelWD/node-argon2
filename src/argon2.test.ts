...
describe("hash", () => {
  ...
  it("with null in password", async () => {
    await expect(
      argon2.hash("pass\0word", { salt })
    ).resolves.toBe(hashes.withNull)
  });
  ...
  it("with secret", async () => {
    await expect(
      argon2.hash("password", { secret, salt })
    ).resolves.toBe(hashes.withSecret)
  });
});
...
describe("needsRehash", () => {
  it("needs rehash old version", async () => {
    ...
    expect(argon2.needsRehash(hash, { version: 0x10 })).toBe(false)
  });
...
describe("verify", () => {
  ...
  it("verify with associated data", async () => {
    await expect(
      argon2.verify(await argon2.hash("password", { associatedData }), "password")
    ).resolves.toBe(true)
  });
  ...
  it("verify with secret", async () => {
    await expect(
      argon2.verify(
        await argon2.hash("password", { secret }),
        "password",
        { secret },
      )
    ).resolves.toBe(true)
  ...
  it.todo("verify old hash format", async () => {
    // older hashes did not contain the v (version) parameter
    await expect(
      argon2.verify(hashes.oldFormat, "password")
    ).resolves.toBe(true)
  });
